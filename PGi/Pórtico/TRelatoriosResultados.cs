...
namespace PG
{
    public class TRelatoriosResultados
    {
        ...
        public string RelatorioAnaliseModal(TPorticoEspacial portico)
        {
            ...
            texto.AppendLine();
            texto.AppendLine("Massa (%) representa o percentual da massa total efetiva que participa no modo de vibração naquela direção global");
          ...
          //  const string formato = "{0,6} | {1,18} | {2,18} | {3,14} | {4,14} | {5,14}";
            string cabecalho = string.Format(formato, "Modo", "Ômega (ω) (rad/s)", "Frequência (Hz)", "Período (s)", "Massa X (%)", "Massa Y (%)", "Massa Z (%)");
            texto.AppendLine(new string('-', cabecalho.Length));
            texto.AppendLine(cabecalho);
          //  texto.AppendLine(string.Format(formato, "Modo", "Omega (rad/s)", "Frequência (Hz)", "Massa X (%)", "Massa Y (%)", "Massa Z (%)"));
            ...
            for (int modo = 0; modo < quantidade; modo++)
            {
                ...
                texto.AppendLine(string.Format(cultura, formato, modo + 1,
                    ...
                    (1.0 / portico.FrequenciasNaturais[modo]).ToString("n6", cultura),
            ...
            texto.AppendLine(new string('-', cabecalho.Length));
            texto.AppendLine(string.Format(formato, "Total", "", "", "",
                acumulado[0].ToString("F4", cultura),
                acumulado[1].ToString("F4", cultura),
                acumulado[2].ToString("F4", cultura)));
